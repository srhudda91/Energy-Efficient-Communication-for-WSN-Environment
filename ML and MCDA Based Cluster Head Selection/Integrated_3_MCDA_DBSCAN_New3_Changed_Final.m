...
%% Define the params structure directly here for use of DDMPEA-ANUM, DWEHC, FOHCA
params = struct();
params.areaSize = 1000; % Area size (square field) [meters]
params.numNodes = 100;  % Number of sensor nodes
params.E0 = Eo;        % Initial energy of each node [Joules]
params.sinkx = 990;     % Sink x-coordinate (center of area)
params.sinky = 990;     % Sink y-coordinate (center of area)
params.packetSize = pkt_size; % Data packet size [bits]
params.ctrlPacketSize = 80; % Control packet size [bits]
params.Eelec = Eelec;   % Energy to run the transmitter/receiver circuitry [J/bit]
params.Eda = EDA;      % Data aggregation energy [J/bit]
params.Efs = 10e-12;    % Free-space model amplifier energy [J/bit/m^2]
params.Emp = Eamp;   % Multipath model amplifier energy [J/bit/m^4]
params.Tx_range = Tx_range;  % Max transmission range (assumed) [meters]
params.alpha = 2;       % Path loss exponent (2 for free-space, 4 for multipath)
params.beta = 1;        % If any other energy model needs beta
params.w1 = 0.7;        % FOHCA: weight for average distance
params.w2 = 0.3;        % FOHCA: weight for energy
...
params.maxGen = 100;     % Maximum number of generations where the DDMPEA-ANUM Algorithms runs

% Common network parameters
params.areaSize  = 990*990;
params.numNodes  = n;
...
params.alpha     = 2;
params.beta      = 1;
params.sink      = [sinkx, sinky];
% DDMPEA-ANUM-specific
params.NP        = 50;
params.maxGen    = 100;
params.P         = 30;
params.sigma1    = 1;
...
%% Step 2: Apply DBSCAN %%
Eps = Tx_range;    % Epsilon neighborhood radius
...
[opNodes_db, dNodes_db, cNodes_db, remainingEnergy_db, abpl_db, lsp_db, nrg_db, threshData_db, cumulativeTx_db, firstNodeDead_db, quarterNodesDead_db, halfNodesDead_db, allNodesDead_db] = runDbscanSimulation(SN, Clust, DistMat, Eo, Eps, MinPts, n, noise_points_count, Eelec, Eamp, EDA, pkt_size, Tx_range, E_thresh);
...
[FigNo, opNodes_roc, dNodes_roc, cNodes_roc, remainingEnergy_roc, abpl_roc, lsp_roc, nrg_roc, threshData_roc, cumulativeTx_roc, firstNodeDead_roc, quarterNodesDead_roc, halfNodesDead_roc, allNodesDead_roc] = runSimulation(Eps, MinPts, FigNo, roc_weights, DistMat, SN, Clust, Eo, n, noise_points_count, Eelec, Eamp, EDA, pkt_size, Tx_range, E_thresh);
...
[FigNo, opNodes_rs, dNodes_rs, cNodes_rs, remainingEnergy_rs, abpl_rs, lsp_rs, nrg_rs, threshData_rs, cumulativeTx_rs, firstNodeDead_rs, quarterNodesDead_rs, halfNodesDead_rs, allNodesDead_rs] = runSimulation(Eps, MinPts, FigNo, rs_weights, DistMat, SN, Clust, Eo, n, noise_points_count, Eelec, Eamp, EDA, pkt_size, Tx_range, E_thresh);
...
[FigNo, opNodes_rr, dNodes_rr, cNodes_rr, remainingEnergy_rr, abpl_rr, lsp_rr, nrg_rr, threshData_rr, cumulativeTx_rr, firstNodeDead_rr, quarterNodesDead_rr, halfNodesDead_rr, allNodesDead_rr] = runSimulation(Eps, MinPts, FigNo, rr_weights, DistMat, SN, Clust, Eo, n, noise_points_count, Eelec, Eamp, EDA, pkt_size, Tx_range, E_thresh);
...
metrics_roc = struct( ...
  ...
  'dNodes',         dNodes_roc, ...
  'cNodes',         cNodes_roc, ...
  'threshData',     threshData_roc, ...
  'cumulativeTx',   cumulativeTx_roc, ...
  'nrg',            nrg_roc, ...
  'remainingEnergy',remainingEnergy_roc, ...
  'abpl',           abpl_roc, ...
  'lsp',            lsp_roc );

accumulateResults('roc', metrics_roc);
logResultsToExcel('roc', metrics_roc, filename);

metrics_rs = struct( ...
  'opNodes',        opNodes_rs, ...
  'dNodes',         dNodes_rs, ...
  'cNodes',         cNodes_rs, ...
  'threshData',     threshData_rs, ...
  'cumulativeTx',   cumulativeTx_rs, ...
  'nrg',            nrg_rs, ...
  'remainingEnergy',remainingEnergy_rs, ...
  'abpl',           abpl_rs, ...
  'lsp',            lsp_rs );

accumulateResults('rs', metrics_rs);
logResultsToExcel('rs', metrics_rs, filename);

metrics_rr = struct( ...
  'opNodes',        opNodes_rr, ...
  'dNodes',         dNodes_rr, ...
  'cNodes',         cNodes_rr, ...
  'threshData',     threshData_rr, ...
  'cumulativeTx',   cumulativeTx_rr, ...
  'nrg',            nrg_rr, ...
  'remainingEnergy',remainingEnergy_rr, ...
  'abpl',           abpl_rr, ...
  'lsp',            lsp_rr );

accumulateResults('rr', metrics_rr);
logResultsToExcel('rr', metrics_rr, filename);

metrics_db = struct( ...
  'opNodes',        opNodes_db, ...
  'dNodes',         dNodes_db, ...
  'cNodes',         cNodes_db, ...
  'threshData',     threshData_db, ...
  'cumulativeTx',   cumulativeTx_db, ...
  'nrg',            nrg_db, ...
  'remainingEnergy',remainingEnergy_db, ...
  'abpl',           abpl_db, ...
  'lsp',            lsp_db );

accumulateResults('db', metrics_db);
logResultsToExcel('db', metrics_db, filename);
...
% figure(31); - done later after the 5 algos are also implemnted
...
function [FigNo, opNodes, dNodes, cNodes, remainingEnergy, abpl, lsp, nrg, threshData, cumulativeTx, firstNodeDead, quarterNodesDead, halfNodesDead, allNodesDead] = runSimulation(Eps, MinPts, FigNo, weights, DistMat, SN, Clust, Eo, n, noise_points_count, Eelec, Eamp, EDA, pkt_size, Tx_range, E_thresh)
opNodes = [];
dNodes = [];
cNodes = [];
threshData = [];
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    cumulativeTx = [cumulativeTx, cumulativeTx(end)+ transmissions];
                ...
                if Clust(i) == 0
                    ...
                    if SN(i).role == 1 % Cluster Head - trianagle with the cluster's colour
                        ...
                        if SN(i).chid > 0 % Ensure that this node has a valid cluster head ID
                            ...
                            plot([SN(i).x, SN(chIndex).x], [SN(i).y, SN(chIndex).y], 'm-'); % Magenta line
                        end
                    end
                end
            else % Dead nodes
    ...
    if quarterNodesDead == -1 && operating_nodes <= (n - noise_points_count)*3/4
        ...
        % Add titles and labels
        title(strcat(titleList(figIndex), 'Network Status When >= 1/4th Nodes Are Dead'));
        xlabel('X-coordinate (Meters)');
        ylabel('Y-coordinate (Meters)');
...
function [opNodes, dNodes, cNodes, remainingEnergy, abpl, lsp, nrg, threshData, cumulativeTx, firstNodeDead, quarterNodesDead, halfNodesDead, allNodesDead] = runDbscanSimulation(SN, Clust, DistMat, Eo, Eps, MinPts, n, noise_points_count, Eelec, Eamp, EDA, pkt_size, Tx_range, E_thresh)
opNodes = [];
dNodes = [];
cNodes = [];
threshData = [];
...
p = 0.1; % a 10 percent of the total amount of nodes used in the network is proposed to give good results
% Number of Clusters %
Ci = p * n;
% Round of Operation %
rounds = 0;
...
for i = 1:n
    ...
    SN(i).tel = 0; % Times elected - states how many times the node was elected as a Cluster Head
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    if (CHeads ~= 0)
        ...
        if rounds == firstRound % Check if it's the first round of clustering
            ...
            for i = 1:n
                if SN(i).role == 1 % Cluster Head
                    plot(SN(i).x, SN(i).y, 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
                else % Normal node (Cluster member)
                    plot(SN(i).x, SN(i).y, 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');
                    % Draw line to the corresponding cluster head
                    if SN(i).chid > 0 % Ensure that this node has a valid cluster head ID
                        chIndex = SN(i).chid; % Get the index of the cluster head
                        plot([SN(i).x, SN(chIndex).x], [SN(i).y, SN(chIndex).y], 'b-'); % blue line
                    end
                end 
            end

            % Plot the sink location
            plot(sinkx, sinky, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');

            % Add titles and labels
            title('HEED: First Cluster Formation');
            xlabel('X-coordinate (Meters)');
            ylabel('Y-coordinate (Meters)');

            % Initialize handles for legend
            hClusterHead = plot(nan, nan, 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
            hClusterMember = plot(nan, nan, 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');
            hSink = plot(nan, nan, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');
            hCLusterLink = plot(nan,nan, 'b-');
            % Create custom legend using handles
            legend([hClusterHead, hClusterMember, hSink, hCLusterLink], {'Cluster Heads', 'Cluster Members', 'Sink', 'Cluster Link'}, ...
                'Location', 'Best');
        ...
        ABPL = calculating_abpl_alternative(CH_list, CHeads);
        ...
        totalTx = 0; % for the number data packet transmissions happening each round
    ...
    if operating_nodes <= n * 0.75 && temp_val == 1
        ...
        for i = 1:n
            if SN(i).condition == 1 % Alive node
                if SN(i).role == 1 % Cluster Head
                    ...
                    plot(SN(i).x, SN(i).y, 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'k');
                    % Draw line to the corresponding cluster head
                    if SN(i).chid > 0 % Ensure that this node has a valid cluster head ID
                        chIndex = SN(i).chid; % Get the index of the cluster head
                        plot([SN(i).x, SN(chIndex).x], [SN(i).y, SN(chIndex).y], 'b-'); % blue line
                ...
                plot(SN(i).x, SN(i).y, 'gx', 'MarkerSize', 4, 'LineWidth', 2); % Black cross for dead nodes
            end
        end
        ...
        % Initialize handles for legend
        hClusterHead = plot(nan, nan, 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
        hClusterMember = plot(nan, nan, 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'k');
        hDeadNode = plot(nan, nan, 'gx', 'MarkerSize', 4, 'LineWidth', 2); % Black cross for dead nodes
        hSink = plot(nan, nan, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');
        hCLusterLink = plot(nan,nan, 'b-');
            
        % Create custom legend using handles
        legend([hClusterHead, hClusterMember, hDeadNode,hSink, hCLusterLink], {'Cluster Heads', 'Cluster Members', 'Dead Nodes', 'Sink', 'Cluster Link'}, ...
                'Location', 'Best');
    ...
    threshData(rounds) = totalTx;
    cumulativeTx_3(rounds) = cumulativeTx_3(end)+ totalTx;
...
metrics_heed = struct( ...
  ...
  'cumulativeTx',   cumulativeTx_3, ...
  'nrg',            nrg, ...
  'remainingEnergy',remainingEnergy, ...
  'abpl',           abpl_3, ...
  'lsp',            lsp_3 );

accumulateResults('heed', metrics_heed);
logResultsToExcel('heed', metrics_heed, filename);
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    if rounds == firstRound % Check if it's the first round of clustering
        ...
        % Plot each node based on its role
            ...
            % Add titles and labels
            title('LEACH-C: First Cluster Formation');
            xlabel('X-coordinate (Meters)');
            ylabel('Y-coordinate (Meters)');
    ...
    if (CLheads ~= 0)
        ...
        ABPL = calculating_abpl_alternative(CL, CLheads);
    ...
    threshData(rounds) = totalTx;
    cumulativeTx_4(rounds) = cumulativeTx_4(end)+totalTx;

    % Get logical index for elements where rleft >= 1
    % idx = [SN.rleft] >= 1;
    % idx = idx';
    % % Subtract 1 from rleft for those elements
    % [SN(idx).rleft] = ([SN(idx).rleft] - 1);

    % Find indices of non-zero elements
    % nonZeroIndices = SN.rleft ~= 0;

    % Subtract 1 from all non-zero elements
    for idx = 1:n
        if(SN(idx).rleft>=1)
            SN(idx).rleft = SN(idx).rleft - 1;
        end
...
metrics_leachc = struct( ...
  'opNodes',        opNodes, ...
  'dNodes',         dNodes, ...
  'cNodes',         cNodes, ...
  'threshData',     threshData, ...
  'cumulativeTx',   cumulativeTx_4, ...
  'nrg',            nrg, ...
  'remainingEnergy',remainingEnergy, ...
  'abpl',           abpl_4, ...
  'lsp',            lsp_4 );

accumulateResults('leachc', metrics_leachc);
logResultsToExcel('leachc', metrics_leachc, filename);
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    if rounds == firstRound % Check if it's the first round of clustering
        ...
        % Plot each node based on its role
            ...
            % Add titles and labels
            title('LEACH: First Cluster Formation');
            xlabel('X-coordinate (Meters)');
            ylabel('Y-coordinate (Meters)');
    ...
    cNodes(rounds) = CLheads;
    threshData(rounds) = totalTx;
    cumulativeTx_5(rounds) = cumulativeTx_5(end)+totalTx;
...
figure(9)
plot(1:length(abpl_5), abpl_5(1:length(abpl_5)), '-b', 'Linewidth', 2);
hold on;

figure(10)
plot(1:length(lsp_5), lsp_5(1:length(lsp_5)), '-b', 'Linewidth', 2);
hold on;

metrics_leach = struct( ...
  'opNodes',        opNodes, ...
  'dNodes',         dNodes, ...
  'cNodes',         cNodes, ...
  'threshData',     threshData, ...
  'cumulativeTx',   cumulativeTx_5, ...
  'nrg',            nrg, ...
  'remainingEnergy',remainingEnergy, ...
  'abpl',           abpl_5, ...
  'lsp',            lsp_5 );

accumulateResults('leach', metrics_leach);
logResultsToExcel('leach', metrics_leach, filename);
...
% abpl_6 = [];
% lsp_6 = [];
% % Special vriables for LEACH-MCDM
% alpha = 0.5; % weight for Residual Energy
% beta = 0.3;  % weight for Distance to Base Station
% gamma = 0.2; % weight for Node Degree
% 
% %-----------Re-initializing values done-----------%
% 
% %%%%%% Set-Up Phase %%%%%%
% fprintf("Starting LEACH-MCDM Clustering, Ended LEACH-MCDM Clustering \n");
% while (operating_nodes > 0 && stop_flag == 0)
% 
%     % Displays Current Round %
%     rounds = rounds + 1
...
% 
%     % Cluster Heads Election %
%     [SN, CL, CLheads] = CH_election_enhanced_LEACH(SN, n, alpha, beta, gamma, rounds, sinkx, sinky, CLheads, Eo);
% 
%     % if no nodes can become CHeads anymore - break out of the loop
%     if(CLheads == 0)
%         break;
...
% 
%         abpl_6(rounds) = ABPL;
%         lsp_6(rounds) = Network_Longest_Shortest_Path;
% 
%     end
%     % After the first clustering happens (inside the while loop for LEACH-MCDM)
%     if rounds == firstRound % Check if it's the first round of clustering
%         % Create a new figure
%         figure(23);
%         hold on;
...
% 
%             % Add titles and labels
%             title('LEACH-MCDM: First Cluster Formation');
%             xlabel('X-coordinate (Meters)');
%             ylabel('Y-coordinate (Meters)');
% 
%             % Initialize handles for legend
...
%             % fprintf("id = %d, CH = %d, ETx = %d, ERx = %d, ETx_to_sink = %d, CHDist = %d, SinkDist = %d\n", SN(i).id, SN(i).chid, ETx , ERx , ETx_to_sink, SN(i).dtch, SN(SN(i).chid).dts);
% 
%             % to make changes to the receiving status we can't directly
%             % use SN(i).chid in CH_list - that's we extract the
%             % position of the CH in CH_list
%             CHindexInCHList = findCHIndex(CL, SN(i).chid);
%             if(CHindexInCHList == -1)
...
%         temp_val = 1;
%         flagFirstDead = rounds;
%         networkLiveData_6(networkStatus) = rounds;
%         networkStatus = networkStatus + 1;
%     end
% 
%     % if operating_nodes <= n * 0.5 && temp_val == 2
...
% 
%         % Add titles and labels
%         title('LEACH-MCDM Network Status When >= 1/4th Nodes Are Dead');
%         xlabel('X-coordinate (Meters)');
%         ylabel('Y-coordinate (Meters)');
% 
%         % Initialize handles for legend
...
% % Plotting Simulation Results "Operating Nodes per Round" %
% figure(3)
% plot(1:length(opNodes), opNodes(1:length(opNodes)), '-m', 'Linewidth', 2);
% hold on;
% 
% figure(4)
% plot(1:length(dNodes), dNodes(1:length(dNodes)), '-m', 'Linewidth', 2);
% hold on;
% 
% figure(5)
% plot(1:length(cNodes), cNodes(1:length(cNodes)), '-m', 'Linewidth', 2);
% hold on;
% 
% figure(6)
% plot(1:length(threshData), threshData(1:length(threshData)), '-m', 'Linewidth', 2);
% hold on;
% 
% figure(7)
% plot(1:length(nrg), nrg(1:length(nrg)), '-m', 'Linewidth', 2);
% hold on;
% 
% figure(8)
% area(1:length(remainingEnergy), remainingEnergy(1:length(remainingEnergy)), 'FaceColor', 'm', 'FaceAlpha', 0.5);
% hold on;
% 
% figure(9)
% plot(1:length(abpl_6), abpl_6(1:length(abpl_6)), '-m', 'Linewidth', 2);
% hold on;
% 
% figure(10)
% plot(1:length(lsp_6), lsp_6(1:length(lsp_6)), '-m', 'Linewidth', 2);
% hold on;
% 
% 

%% DDMPEA-ANUM
fprintf("Starting Simulation for DDMPEA-ANUM\n");
...
[opNodes_ddmpea, dNodes_ddmpea, cNodes_ddmpea, remainingEnergy_ddmpea, abpl_ddmpea, lsp_ddmpea, nrg_ddmpea, threshData_ddmpea, cumulativeTx_ddmpea, firstNodeDead_ddmpea, quarterNodesDead_ddmpea, halfNodesDead_ddmpea, allNodesDead_ddmpea] = runDDMPEA_Simulation(SN_reset, DistMat, params_ddmpea);

metrics_ddmpea = struct( ...
  'opNodes',        opNodes_ddmpea, ...
  'dNodes',         dNodes_ddmpea, ...
  'cNodes',         cNodes_ddmpea, ...
  'threshData',     threshData_ddmpea, ...
  'cumulativeTx',   cumulativeTx_ddmpea, ...
  'nrg',            nrg_ddmpea, ...
  'remainingEnergy',remainingEnergy_ddmpea, ...
  'abpl',           abpl_ddmpea, ...
  'lsp',            lsp_ddmpea );

accumulateResults('ddmpea', metrics_ddmpea);
logResultsToExcel('ddmpea', metrics_ddmpea, filename);
...
SN_reset = resetNodes(n, Eo, sinkx, sinky, pos, DistMat, Eps);
[opNodes_dwehc, dNodes_dwehc, cNodes_dwehc, remainingEnergy_dwehc, abpl_dwehc, lsp_dwehc, nrg_dwehc, threshData_dwehc, cumulativeTx_dwehc, firstNodeDead_dwehc, quarterNodesDead_dwehc, halfNodesDead_dwehc, allNodesDead_dwehc] = runDWEHC_Simulation(SN_reset, DistMat, params_dwehc);

metrics_dwehc = struct( ...
  'opNodes',        opNodes_dwehc, ...
  'dNodes',         dNodes_dwehc, ...
  'cNodes',         cNodes_dwehc, ...
  'threshData',     threshData_dwehc, ...
  'cumulativeTx',   cumulativeTx_dwehc, ...
  'nrg',            nrg_dwehc, ...
  'remainingEnergy',remainingEnergy_dwehc, ...
  'abpl',           abpl_dwehc, ...
  'lsp',            lsp_dwehc );

accumulateResults('dwehc', metrics_dwehc);
logResultsToExcel('dwehc', metrics_dwehc, filename);
...
SN_reset = resetNodes(n, Eo, sinkx, sinky, pos, DistMat, Eps);
[opNodes_fohca, dNodes_fohca, cNodes_fohca, remainingEnergy_fohca, abpl_fohca, lsp_fohca, nrg_fohca, threshData_fohca, cumulativeTx_fohca, firstNodeDead_fohca, quarterNodesDead_fohca, halfNodesDead_fohca, allNodesDead_fohca] = runFOHCA_Simulation(SN_reset, DistMat, params_fohca);

metrics_fohca = struct( ...
  'opNodes',        opNodes_fohca, ...
  'dNodes',         dNodes_fohca, ...
  'cNodes',         cNodes_fohca, ...
  'threshData',     threshData_fohca, ...
  'cumulativeTx',   cumulativeTx_fohca, ...
  'nrg',            nrg_fohca, ...
  'remainingEnergy',remainingEnergy_fohca, ...
  'abpl',           abpl_fohca, ...
  'lsp',            lsp_fohca );

accumulateResults('fohca', metrics_fohca);
logResultsToExcel('fohca', metrics_fohca, filename);

%% Figure plotting for DDMPEA-ANUM, DWEHC & FOHCA
colour_DDMPEA = '#0072BD'; % Deep blue
colour_DWEHC  = '#D95319'; % Bright orange
colour_FOHCA  = '#9E7E9E'; % Light purple

% % legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');

figure(3)
hold on;
plot(1:length(opNodes_ddmpea), opNodes_ddmpea, '-', 'LineWidth', 2, 'Color', colour_DDMPEA);
plot(1:length(opNodes_dwehc), opNodes_dwehc, '-', 'LineWidth', 2, 'Color', colour_DWEHC);
plot(1:length(opNodes_fohca), opNodes_fohca, '-', 'LineWidth', 2, 'Color', colour_FOHCA);
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best'); legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
hold off;

figure(4)
hold on;
plot(1:length(dNodes_ddmpea), dNodes_ddmpea(1:length(dNodes_ddmpea)), '-', 'LineWidth', 2, 'Color', colour_DDMPEA);
plot(1:length(dNodes_dwehc), dNodes_dwehc(1:length(dNodes_dwehc)), '-', 'LineWidth', 2, 'Color', colour_DWEHC);
plot(1:length(dNodes_fohca), dNodes_fohca(1:length(dNodes_fohca)), '-', 'LineWidth', 2, 'Color', colour_FOHCA);
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best'); legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
hold off;
...
figure(5)
hold on;
plot(1:length(cNodes_ddmpea), cNodes_ddmpea(1:length(cNodes_ddmpea)), '-', 'LineWidth', 2, 'Color', colour_DDMPEA);
plot(1:length(cNodes_dwehc), cNodes_dwehc(1:length(cNodes_dwehc)), '-', 'LineWidth', 2, 'Color', colour_DWEHC);
plot(1:length(cNodes_fohca), cNodes_fohca(1:length(cNodes_fohca)), '-', 'LineWidth', 2, 'Color', colour_FOHCA);
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best'); legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
hold off;
...
figure(6)
hold on;
plot(1:length(threshData_ddmpea), threshData_ddmpea(1:length(threshData_ddmpea)), '-', 'LineWidth', 2, 'Color', colour_DDMPEA);
plot(1:length(threshData_dwehc), threshData_dwehc(1:length(threshData_dwehc)), '-', 'LineWidth', 2, 'Color', colour_DWEHC);
plot(1:length(threshData_fohca), threshData_fohca(1:length(threshData_fohca)), '-', 'LineWidth', 2, 'Color', colour_FOHCA);
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best'); legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
hold off;
...
figure(7)
hold on;
plot(1:length(nrg_ddmpea), nrg_ddmpea(1:length(nrg_ddmpea)), '-', 'LineWidth', 2, 'Color', colour_DDMPEA);
plot(1:length(nrg_dwehc), nrg_dwehc(1:length(nrg_dwehc)), '-', 'LineWidth', 2, 'Color', colour_DWEHC);
plot(1:length(nrg_fohca), nrg_fohca(1:length(nrg_fohca)), '-', 'LineWidth', 2, 'Color', colour_FOHCA);
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best'); legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
hold off;
...
figure(8)
hold on;
area(1:length(remainingEnergy_ddmpea), remainingEnergy_ddmpea, 'FaceAlpha', 0.5, 'FaceColor', hex2rgb('#0072BD')); % DDMPEA-ANUM (Deep Blue)
area(1:length(remainingEnergy_dwehc), remainingEnergy_dwehc, 'FaceAlpha', 0.5, 'FaceColor', hex2rgb('#D95319'));  % DWEHC (Bright Orange)
area(1:length(remainingEnergy_fohca), remainingEnergy_fohca, 'FaceAlpha', 0.5, 'FaceColor', hex2rgb('#9E7E9E'));  % FOHCA (Light Purple)

% legend(flip({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}), 'Location', 'Best');
legend(flip({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}), 'Location', 'Best');
hold off;

figure(9)
hold on;
plot(1:length(abpl_ddmpea), abpl_ddmpea(1:length(abpl_ddmpea)), '-', 'LineWidth', 2, 'Color', colour_DDMPEA);
plot(1:length(abpl_dwehc), abpl_dwehc(1:length(abpl_dwehc)), '-', 'LineWidth', 2, 'Color', colour_DWEHC);
plot(1:length(abpl_fohca), abpl_fohca(1:length(abpl_fohca)), '-', 'LineWidth', 2, 'Color', colour_FOHCA);
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best'); legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
hold off;
...
figure(10)
hold on;
plot(1:length(lsp_ddmpea), lsp_ddmpea(1:length(lsp_ddmpea)), '-', 'LineWidth', 2, 'Color', colour_DDMPEA);
plot(1:length(lsp_dwehc), lsp_dwehc(1:length(lsp_dwehc)), '-', 'LineWidth', 2, 'Color', colour_DWEHC);
plot(1:length(lsp_fohca), lsp_fohca(1:length(lsp_fohca)), '-', 'LineWidth', 2, 'Color', colour_FOHCA);
% legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best'); legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'Best');
hold off;

% helper function to convert hexadecimal to RGB for area plot
...
% For just 3+MCDA new 3 Algos of all Algos
x = categorical({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'});
x = reordercats(x, {'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'});
...
%% subplots
% Define your algorithms & display names
algoTags  = {'roc','rs','rr','db','heed','leachc','leach','ddmpea','dwehc','fohca'};
algoNames = {'RCA','SRA','RRA','DBSCAN','HEED','LEACH-C','LEACH', ...
             'DDMPEA-ANUM','DWEHC','FOHCA'};

% Pick a color for each (10×3 matrix of RGBs, or e.g. lines(10))
myColors = {hex2rgb(colour_RCA), hex2rgb(colour_SRA), hex2rgb(colour_RRA), hex2rgb(colour_DB), 'g', 'c', 'b', hex2rgb(colour_DDMPEA), hex2rgb(colour_DWEHC), hex2rgb(colour_FOHCA)};    

% Once all metrics are in __resultsMap__:
plotAllSubplots(algoTags, algoNames, myColors);
disp("All subplots displayed");

%% ALL FUNCTIONS %%
% logging to excel function
function logResultsToExcel(tag,metrics,filename)
% Write whatever fields exist in metrics to an Excel sheet named tag
%   tag      = sheet name (≤31 chars)
%   metrics  = struct with fields like .opNodes, .dNodes, .cNodes, .lsp, etc.
%   filename = 'your_workbook.xlsx'

  % Build variable names & data from the fields that exist
  fn = fieldnames(metrics);
  T = table((1:max(cellfun(@numel,struct2cell(metrics))))', 'VariableNames', {'Round'});
  for i = 1:numel(fn)
    v = metrics.(fn{i});
    if isnumeric(v)
  ...
  % Write (or overwrite) into the specified sheet
  writetable(T, filename, 'Sheet', tag(1:min(end,31)), 'WriteRowNames', false);
end

%% accumulating metrics into results
function accumulateResults(tag, metrics)
% Store each algorithm’s metrics into a persistent map
  persistent resultsMap
  if isempty(resultsMap)
    resultsMap = containers.Map;
  end
  resultsMap(tag) = metrics;
  assignin('base','resultsMap',resultsMap);
end

%% plotting all subplots
function plotAllSubplots(algoOrder, algoNames, colors)
% plotAllSubplots  Draws one 2×5 figure per metric,  
%   with each subplot showing exactly one algorithm in your chosen color,
%   and a legend naming that algorithm.
%
%   INPUTS:
%     algoOrder : cell array of tags, e.g. {'roc','rs',…,'fohca'}
%     algoNames : cell array of display names, same size as algoOrder
%     colors    : N×3 numeric array of RGBs (or N-element cell of MATLAB color specs)

  % Retrieve the stored results
  resultsMap = evalin('base','resultsMap');

  % The eight metrics & titles we’ll plot
  metrics    = {'opNodes','dNodes','cNodes','threshData', 'cumulativeTx', ...
    'nrg', 'remainingEnergy','abpl','lsp'};
  metricTitles = { ...
    'Operating Nodes Per Round', ...
    'Dead Nodes Per Round', ...
    'Clusters Per Round', ...
    'Transmissions Per Round', ...
    'Cumulative Transmission Uptill This Round', ...
    'Energy Per Transmission', ...
    'Remaining Energy Per Round', ...
    'Average Backbone Path Length', ...
    'Longest Shortest Path Length'};

  nAlgos = numel(algoOrder);

  for m = 1:numel(metrics)
    figure;  % one figure per metric
    for a = 1:nAlgos
      tag   = algoOrder{a};
      name  = algoNames{a};
      if ~isKey(resultsMap, tag)
        continue;  % skip any alg that never ran
      end

      % pull the vector for this algorithm & metric
      data = resultsMap(tag).(metrics{m});
      ax = subplot(2,5,a);
      hold(ax,'on');

      % choose the color for this algo
      if iscell(colors)
        col = colors{a};
      else
        col = colors(a,:);
      ...
      % plot it
      if strcmp(metrics{m},'remainingEnergy')
        area(ax, 1:numel(data), data, 'FaceColor', col, 'FaceAlpha', 0.5,'EdgeColor','none');
      else
        plot(ax, 1:numel(data), data, '-', 'Color', col, 'LineWidth',1.5);
      end

      grid(ax,'on');
      title(ax, name, 'FontWeight','bold');

      % Add a little legend in each subplot
      legend(ax, name, 'Location','best','TextColor',col);

      % only label outer axes
      if a>5, xlabel(ax,'Round'); end
      if mod(a-1,5)==0, ylabel(ax, metricTitles{m}); end

      hold(ax,'off');
    end

    sgtitle(metricTitles{m});
  end
end
...
%% algorithm for LEACH-MCDM
function [SN, CL, CLheads] = CH_election_enhanced_LEACH(SN, n, alpha, beta, gamma, rounds, sinkx, sinky, CLheads, Eo)
CL = [];
...
for i = 1:n
    ...
    if (SN(i).Energy > 0)
        % Calculate node degree (number of neighbors within a radius)
        commRange = 100; % you can define it globally or pass if needed
        k_n = 0;
        for j = 1:n
            if i ~= j
                dist_ij = sqrt((SN(i).x - SN(j).x)^2 + (SN(i).y - SN(j).y)^2);
                if dist_ij <= commRange
                    k_n = k_n + 1;
                end
            end
        end

        % Find max values across network
        Emax = max([SN(:).Energy]);
        dmax = sqrt((sinkx)^2 + (sinky)^2); % assuming sink is at (sinkx, sinky)
        kmax = max(n-1, 1); % worst case node degree, avoid divide by zero

        % Calculate T(n) for this node
        T_n = alpha * (SN(i).Energy / Emax) + ...
            beta * (dmax - sqrt((SN(i).x - sinkx)^2 + (SN(i).y - sinky)^2)) / dmax + ...
            gamma * (k_n / kmax);
...
%% fitness function for ddmpea 
function f = fitness_DDMPEA(sol, positions, energy, sink)
% Fuzzy membership-based fitness calculation
selected = sol > 0.5;
...
% Distance to sink for CHs
if ~isempty(CH)
    distances = sqrt(sum((positions(CH,:) - sinkPos).^2, 2));
    distance_obj = mean(distances);
else
    distance_obj = inf;
end

% Energy objective (sum of energies of CHs)
energy_obj = sum(energy(selected));

% Normalize objectives (fuzzy membership)
E_min = min(energy);
E_max = max(energy);
mu_energy = (energy_obj - E_min) / (E_max - E_min + eps);

D_all = sqrt(sum((positions - sinkPos).^2,2));
D_min = min(D_all);
D_max = max(D_all);
mu_distance = 1 - (distance_obj - D_min) / (D_max - D_min + eps);

% Weighted fuzzy combination
w1 = 0.7; w2 = 0.3;
combined = w1 * mu_energy + w2 * mu_distance;

f = 1 / (combined + eps); % Minimization
end


%% New Simulation Driver for DDMPEA-ANUM
function [opNodes_ddmpea, dNodes_ddmpea, cNodes_ddmpea, remainingEnergy_ddmpea, abpl_ddmpea, lsp_ddmpea, nrg_ddmpea, threshData_ddmpea, cumulativeTx_ddmpea, firstNodeDead_ddmpea, quarterNodesDead_ddmpea, halfNodesDead_ddmpea, allNodesDead_ddmpea] = runDDMPEA_Simulation(SN, DistMat, params)
n = params.numNodes;
noise_points_count = 0; % DDMPEA-ANUM does not use noise
rounds = 0;
...
while sum(energy > 0) > 0
    ...
    [energy, dataSent] = calculateEnergy_ddmpea(positions, energy, CH, clusters, params);

    aliveNodes = sum(energy > 0);
    deadNodes = n - aliveNodes;
    fprintf('DDMPEA-ANUM - Round %d: Alive Nodes = %d, Total Energy = %.4f J\n', rounds, aliveNodes, sum(energy));

    if rounds == 1
        firstCHs_ddmpea = CH;
        firstClusters_ddmpea = clusters;
        figure(23);
        hold on;
        grid on;
        % axis([0 params.areaSize 0 params.areaSize]);
        title('DDMPEA-ANUM: First Cluster Formation');

        % Plot all sensor nodes
        plot(positions(:,1), positions(:,2), 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');

        % Plot the sink
        plot(params.sinkx, params.sinky, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');

        % Plot Cluster Heads
        plot(positions(firstCHs_ddmpea,1), positions(firstCHs_ddmpea,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');

        % Draw lines from cluster members to their CH
        for i = 1:length(firstClusters_ddmpea)
            members = firstClusters_ddmpea{i};
            CH = firstCHs_ddmpea(i);
            for j = 1:length(members)
                plot([positions(CH,1), positions(members(j),1)], [positions(CH,2), positions(members(j),2)], 'b-');
            end
        end

        xlabel('X-coordinate (Meters)', 'FontSize', 12);
        ylabel('Y-coordinate (Meters)', 'FontSize', 12);
        % Initialize handles for legend
        hClusterHead = plot(nan, nan, 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
        hClusterMember = plot(nan, nan, 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');
        hSink = plot(nan, nan, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');
    ...
    if quarterNodesDead_ddmpea == -1 && deadNodes >= 0.25 * (n - noise_points_count)
        ...
        aliveIdx = find(energy > 0); % alive nodes indices
        quarterCHs_ddmpea = CH(ismember(CH, aliveIdx)); % only alive cluster heads
        quarterClusters_ddmpea = clusters;
        figure(24);
        hold on;
        grid on;
        % axis([0 params.areaSize 0 params.areaSize]);
        title(['DDMPEA-ANUM Network Status When', ...
            newline, ... 
            '>= 1/4th Nodes Are Dead']);

        % Plot alive sensor nodes
        aliveNodesIdx = find(energy > 0);
        plot(positions(aliveNodesIdx,1), positions(aliveNodesIdx,2), 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'k');

        % Plot the sink
        plot(params.sinkx, params.sinky, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');

        % Plot Cluster Heads at 25% death
        plot(positions(quarterCHs_ddmpea,1), positions(quarterCHs_ddmpea,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');

        % Plot dead sensor nodes
        deadNodesIdx = find(energy <= 0);
        plot(positions(deadNodesIdx,1), positions(deadNodesIdx,2), 'gx', 'MarkerSize', 4, 'LineWidth', 2);

        % Draw lines from cluster members to their CH
        nClusters = min(length(quarterClusters_ddmpea), length(quarterCHs_ddmpea));
        for i = 1:nClusters
            members = quarterClusters_ddmpea{i};
            CH = quarterCHs_ddmpea(i);
            for j = 1:length(members)
                plot([positions(CH,1), positions(members(j),1)], [positions(CH,2), positions(members(j),2)], 'b-');
            end
        end
    ...
    cumulativeTx_ddmpea = [cumulativeTx_ddmpea, cumulativeTx_ddmpea(end) + dataSent];
...
%% DWEHC Function
function result = DWEHC(positions, E_res, E_init, R, alpha)
% DWEHC implements the Distributed Weight-based Energy-Efficient Hierarchical Clustering
% positions: N x 2 matrix of node (x,y) coordinates
% E_res: N x 1 vector of residual energies
% E_init: scalar or N x 1 vector of initial energies
% R: cluster range (scalar)
% alpha: path-loss exponent (e.g., 2 or 4)

N = size(positions,1);

if isscalar(E_init)
    E_init = E_init * ones(N,1);  % Expand scalar E_init into N×1 vector
end

% 1) Find neighbors
neighbors = cell(N,1);
for s=1:N
    neighbors{s} = findNeighbors(s, positions, R, alpha);
end
...
for s=1:N
    ...
    if isempty(Ns)
        ...
        dists = sqrt(sum((positions(Ns,:) - positions(s,:)).^2, 2));
        weights(s) = (E_res(s) ./ E_init(s)) * sum((R - dists) ./ (6 * R));
    end
end

% 3) Initialize clustering variables
my_level = -1 * ones(N,1);
my_temp_head = false(N,1);
my_head_num = zeros(N,1);
my_dis = inf(N,1);
head_id = zeros(N,1);
head_x = zeros(N,1);
head_y = zeros(N,1);
...
% 4) Cluster generation iterations
for i = 0:MAX_ITER
    % Temporary head election
    for s = 1:N
        if my_level(s) == -1
            Ns = neighbors{s};
            Ns = Ns(:);
            cand_ids = [s; Ns];
            [~, idx] = max(weights(cand_ids));
            leader = cand_ids(idx);
            my_temp_head(s) = (leader == s);
            my_head_num(s) = leader;
        end
    end
    ...
    for s = 1:N
        ...
        if isempty(Ns)
            ...
            percent(s) = sum(my_head_num(Ns) == s) / numel(Ns);
        end
    end
    threshold = (MAX_ITER - i) / MAX_ITER;

    % Real clusterhead selection
    for s = 1:N
        Ns = neighbors{s};
        if my_level(s) == -1
            cond1 = my_temp_head(s) && (percent(s) >= threshold);
            cond2 = isempty(Ns);
            cond3 = all(my_level(Ns) > -1);
            if cond1 || cond2 || cond3
                my_level(s) = 0;
                my_dis(s) = 0;
                head_id(s) = s;
                head_x(s) = positions(s,1);
                head_y(s) = positions(s,2);
            end
        end
    end
    ...
    for s = 1:N
        if my_level(s) ~= 0
            ...
            for k = 1:numel(Ns)
                v = Ns(k);
                if head_id(v) ~= 0
                    dx = head_x(v) - positions(s,1);
                    dy = head_y(v) - positions(s,2);
                    range_new = sqrt(dx^2 + dy^2);
                    dist_sv = norm(positions(v,:) - positions(s,:));
                    dis_new = my_dis(v) + dist_sv;

                    shouldJoinOrSwitch = (my_level(s) > 0 && dis_new < my_dis(s)) || ...
                        (my_level(s) == -1 && range_new <= R);
                    if shouldJoinOrSwitch
...
% 6) Build neighbor lists for each clusterhead
cluster_heads = unique(head_id(head_id > 0));
neighbors_list = cell(length(cluster_heads),1);
for i = 1:length(cluster_heads)
    ch = cluster_heads(i);
    members = find(head_id == ch);
    neighbors_list{i} = members;
end

% 7) Package results
result.head_id = head_id;
result.level = my_level;
result.parent = my_parent;
result.weight = weights;
result.neighbors = neighbors_list;

end

%% DWEHC Helper: Find Neighbors
function N = findNeighbors(s, positions, R, alpha)
N = [];
allIdx = setdiff(1:size(positions,1), s);
dists = sqrt(sum((positions(allIdx,:) - positions(s,:)).^2, 2));
Q = allIdx(dists <= R);
[~, ord] = sort(dists(dists <= R));
Q = Q(ord);
...
while ~isempty(Q)
    ...
    for i = 1:numel(Q)
        ...
        sx = norm(positions(x,:) - positions(s,:))^alpha;
        sv = norm(positions(v,:) - positions(s,:))^alpha;
        vx = norm(positions(x,:) - positions(v,:))^alpha;
        if sx < sv + vx
            tk(i) = true;
        end
    end
...
function [opNodes_dwehc, dNodes_dwehc, cNodes_dwehc, remainingEnergy_dwehc, abpl_dwehc, lsp_dwehc, nrg_dwehc, threshData_dwehc, cumulativeTx_dwehc, firstNodeDead_dwehc, quarterNodesDead_dwehc, halfNodesDead_dwehc, allNodesDead_dwehc] = runDWEHC_Simulation(SN, DistMat, params)
...
while sum(energy > 0) > 0
    ...
    aliveIdx = find(energy > 0); % Alive nodes (global indices)
    posAlive = positions(aliveIdx, :);
    EresAlive = energy(aliveIdx);

    result = DWEHC(posAlive, EresAlive, params.Einit, params.R, params.alpha);
    CH = unique(result.head_id);
    valueToRemove = 0;
    CH(CH == valueToRemove) = [];

    clusters = result.neighbors;
    [energy, dataSent, CH] = calculateEnergy_generic(positions, energy, CH, clusters, params);
    % if the number of cluster heads reduce to 0, no mroe transmissions
    if numel(CH) <= 0
        if halfNodesDead_dwehc == -1 
    ...
    if rounds == 1
        % Plot First Cluster Formation
        figure(25); hold on; grid on;
        title('DWEHC: First Cluster Formation');
        plot(positions(:,1), positions(:,2), 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');
        plot(params.sinkx, params.sinky, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');
        heads = find(result.level == 0);
        plot(positions(heads,1), positions(heads,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
        
        for s = 1:n
            if result.parent(s) > 0
                p = result.parent(s);
                plot([positions(s,1), positions(p,1)], [positions(s,2), positions(p,2)], 'b-');
            end
        end
        xlabel('X-coordinate (Meters)');
        ylabel('Y-coordinate (Meters)');
    ...
    if quarterNodesDead_dwehc == -1 && deadNodes >= 0.25 * n
        quarterNodesDead_dwehc = rounds;
        
        figure(26); hold on; grid on;
        title('DWEHC: Network Status at 25% Nodes Dead');
        aliveIdx = find(energy > 0);
        plot(positions(aliveIdx,1), positions(aliveIdx,2), 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');
        plot(params.sinkx, params.sinky, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');
        heads_local = find(result.level == 0);
        valid_local_heads = heads_local(heads_local <= length(aliveIdx));
        heads = aliveIdx(valid_local_heads);       % Map local to global indices

        if ~isempty(heads_local)
            plot(positions(heads,1), positions(heads,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
        end
        plot(positions(heads,1), positions(heads,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
        
        % for s = aliveIdx'
        %     if result.parent(s) > 0 && energy(result.parent(s)) > 0
        %         plot([positions(s,1), positions(result.parent(s),1)], [positions(s,2), positions(result.parent(s),2)], 'b-');
        %     end
        % end
        % Draw parent-child links
        for i = 1:numel(aliveIdx)
            s = aliveIdx(i);  % Global node index for alive node

            if result.parent(i) > 0 && result.parent(i) <= numel(aliveIdx)
                p_local_idx = result.parent(i);
                p = aliveIdx(p_local_idx);  % Map local parent to global index

                % Check if parent is still alive
                if ismember(p, aliveIdx) && energy(p) > 0
                    plot([positions(s,1), positions(p,1)], [positions(s,2), positions(p,2)], 'b-');
                end
            end
        end
    ...
    if halfNodesDead_dwehc == -1 && deadNodes >= 0.5 * n
        halfNodesDead_dwehc = rounds;
    end
    if aliveNodes == 0
        allNodesDead_dwehc = rounds;
    ...
    cumulativeTx_dwehc = [cumulativeTx_dwehc, cumulativeTx_dwehc(end) + dataSent];
...
for iter = 1:FO_iter
    ...
    for c = 1:k
        ...
        for m = members'
            ...
            fit = w1*avgDist - w2*avgEnergy + (1-w1-w2)*(1/numMembers);
            if fit < bestFit
                bestFit = fit;
                bestNode = m;
            end
...
function [opNodes_fohca, dNodes_fohca, cNodes_fohca, remainingEnergy_fohca, abpl_fohca, lsp_fohca, nrg_fohca, threshData_fohca, cumulativeTx_fohca, firstNodeDead_fohca, quarterNodesDead_fohca, halfNodesDead_fohca, allNodesDead_fohca] = runFOHCA_Simulation(SN, DistMat, params)
n = params.numNodes;
...
while sum(energy > 0) > 0
    ...
    if rounds == 1
        firstCHs_fohca = CH;          % Save first CHs
        firstClusters_fohca = clusters; % Save first Clusters
        figure(27);
        hold on;
        grid on;
        % axis([0 params.areaSize 0 params.areaSize]);
        title('FOHCA: First Cluster Formation');
        ...
        % Plot Cluster Heads
        plot(positions(firstCHs_fohca,1), positions(firstCHs_fohca,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');

        % Draw lines from cluster members to their CH
        nPairs = min(numel(firstCHs_fohca), numel(firstClusters_fohca));
        for i = 1:nPairs
            members = firstClusters_fohca{i};
            CH = firstCHs_fohca(i);
            for j = 1:length(members)
                plot([positions(CH,1), positions(members(j),1)], [positions(CH,2), positions(members(j),2)], 'b-');
            end
    ...
    if quarterNodesDead_fohca == -1 && deadNodes >= 0.25 * (n - noise_points_count)
        ...
        aliveIdx = find(energy > 0); % alive nodes indices
        quarterCHs_fohca = CH(ismember(CH, aliveIdx)); % only alive cluster heads
        quarterClusters_fohca = clusters; % Save quarter-dead Clusters
        figure(28);
        hold on;
        grid on;
        % axis([0 params.areaSize 0 params.areaSize]);
        title('FOHCA Network Status When >= 1/4th Nodes Are Dead');

        % Plot alive sensor nodes
        aliveNodesIdx = find(energy > 0);
        plot(positions(aliveNodesIdx,1), positions(aliveNodesIdx,2), 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');

        % Plot the sink
        plot(params.sinkx, params.sinky, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');

        % Plot Cluster Heads at 25% death
        plot(positions(quarterCHs_fohca,1), positions(quarterCHs_fohca,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
        ...
        % Draw lines from cluster members to their CH
        nClusters = min(length(quarterClusters_fohca), length(quarterCHs_fohca));
        for i = 1:nClusters
            members = quarterClusters_fohca{i};
            CH = quarterCHs_fohca(i);
            for j = 1:length(members)
                plot([positions(CH,1), positions(members(j),1)], [positions(CH,2), positions(members(j),2)], 'b-');
            end
        end
    ...
    cumulativeTx_fohca = [cumulativeTx_fohca, cumulativeTx_fohca(end) + dataSent];
    totalEnergy = sum(energy);
end
end

%% calculates energy for the DWEHC & FOHCA
function [energyUpdated, dataSent, newCHs] = calculateEnergy_generic(positions, energy, CHs, clusters, params)
dataSent = 0;
newCHs = [];
for i = 1:numel(CHs)
    head = CHs(i);
    ...
    for m = members(:)'
        if m==head || head == 0, continue; end
        d = norm(positions(m,:)-positions(head,:));
        % Etx = params.Eelec*params.packetSize + params.Efs*d^2*params.packetSize;
        if d <= sqrt(params.Efs/params.Emp)
            Etx = params.Eelec * params.packetSize + params.Efs * d^2 * params.packetSize;
        else
            Etx = params.Eelec * params.packetSize + params.Emp * d^2 * params.packetSize;
        end
        energy(m)   = energy(m) - Etx;
        if energy(m) <= 0
            % fprintf("energy(%f) <= 0\n", m);
            continue;   % Skip if member dead after transmission
        end
    ...
    if(energy(head)>0)
        ...
        if dBS <= sqrt(params.Efs/params.Emp)
            EtxBS = params.Eelec*params.packetSize + params.Efs*dBS^2*params.packetSize;
        else
            EtxBS = params.Eelec*params.packetSize + params.Emp*dBS^2*params.packetSize;
        end
        energy(head) = energy(head) - EtxBS;
        dataSent = dataSent + params.packetSize;
    end

    % After entire round, check if head is still alive
    if energy(head) > 0
        newCHs(end+1) = head;  % Keep head if still alive
    end
...
%% calculates energy in case of DDMPEA
function [energyUpdated, dataSent] = calculateEnergy_ddmpea(positions, energy, CHs, clusters, params)
% Energy dissipation model
sink = [params.sinkx, params.sinky];
dataSent = 0;
...
for i = 1:length(CHs)
    ...
    % --- Cluster Head Energy ---
    recv_energy = length(members) * params.packetSize * params.Eelec;
    agg_energy = length(members) * params.packetSize * params.Eda;

    d_sink = norm(positions(ch,:) - sink);
    tx_energy = params.packetSize * (params.Eelec + params.Efs * d_sink^2);
    % if d_sink <= sqrt(params.Efs/params.Emp)
    %     tx_energy = params.packetSize * (params.Eelec + params.Efs * d_sink^2);
    % else
    %     tx_energy = params.packetSize * (params.Eelec + params.Emp * d_sink^4);
    % end

    energy(ch) = energy(ch) - (recv_energy + agg_energy + tx_energy);
    dataSent = dataSent + params.packetSize;

    % --- Cluster Members Energy ---
    for j = 1:length(members)
        member = members(j);
        d_member = norm(positions(member,:) - positions(ch,:));

        if d_member <= sqrt(params.Efs/params.Emp)
            tx_member_energy = params.packetSize * (params.Eelec + params.Efs * d_member^2);
        else
            tx_member_energy = params.packetSize * (params.Eelec + params.Emp * d_member^4);
        end

        energy(member) = energy(member) - tx_member_energy;
    end
end
...
% alternative way to calculate the Average backbone path length for each CH
%  lsp_path is the path taken here when traversing the via multihop from
%  CH to sink - the mean of those paths will be abpl and the max is lsp
function abpl = calculating_abpl_alternative(CH_list, CHeads)
abpl = -1;
    ...
    if CH_list(i).lsp_path(1) == 0
        ...
        for j = 1:length(CH_list(i).lsp_path)
            ...
            if CH_list(i).lsp_path(j) == 0
                ...
                CH_list(i).abpl = CH_list(i).abpl + (sqrt((CH_list(i).x - CH_list(CH_list(i).lsp_path(j)).x) ^ 2 + (CH_list(i).y - CH_list(CH_list(i).lsp_path(j)).y) ^ 2));
