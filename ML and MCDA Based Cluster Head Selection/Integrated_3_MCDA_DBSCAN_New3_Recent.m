...
figure(2);
hold on;
title 'First Clustering Based on DBSCAN';
xlabel 'X-coordinate (Field Size in Meters)';
ylabel 'Y-coordinate (Field Size in Meters)';
...
figure(6);
hold on;
title({'COMPARISON'; ['Number of Transmissions', ...
            newline, ... 
            'Occurring Per Round']});
xlabel '# Rounds';
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    if rounds == 1
        ...
        title(strcat(titleList(figIndex), ['Network Status After', ...
            newline, ... 
            'First Cluster Head Election']));
    ...
    if quarterNodesDead == -1 && operating_nodes <= (n - noise_points_count)*3/4
        ...
        % Add titles and labels
        title(strcat(titleList(figIndex), ['Network Status When', ...
            newline, ... 
            '>= 1/4th Nodes aAre Dead']));
        xlabel('X-coordinate (Meters)');
        ylabel('Y-coordinate (Meters)');
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    if operating_nodes <= n * 0.75 && temp_val == 1
        ...
        % Add titles and labels
        title(['HEED Network Status When', ...
            newline, ... 
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    if operating_nodes <= n * 0.75 && temp_val == 1
        ...
        % Add titles and labels
        title(['LEACH-C Network Status When', ...
            newline, ... 
            '>= 1/4th Nodes Are Dead']);
        xlabel('X-coordinate (Meters)');
        ylabel('Y-coordinate (Meters)');
...
while (operating_nodes > 0 && stop_flag == 0)
    ...
    if operating_nodes <= n * 0.75 && temp_val == 1
        ...
        % Add titles and labels
        title(['LEACH Network Status When', ...
            newline, ... 
            '>= 1/4th Nodes Are Dead']);
        xlabel('X-coordinate (Meters)');
        ylabel('Y-coordinate (Meters)');
...
while sum(energy > 0) > 0
    ...
    if quarterNodesDead_dwehc == -1 && deadNodes >= 0.25 * n
        ...
        title(['DWEHC: Network Status When', ...
            newline, ... 
            '>=1/4th Nodes Are Dead']);
...
while sum(energy > 0) > 0
    ...
    if rounds == 1
        ...
        % Draw lines from cluster members to their CH
        for i = 1:length(firstClusters_fohca)
    ...
    if quarterNodesDead_fohca == -1 && deadNodes >= 0.25 * (n - noise_points_count)
        ...
        grid on;
        % axis([0 params.areaSize 0 params.areaSize]);
        title(['FOHCA Network Status When', ...
            newline, ... 
            '>= 1/4th Nodes Are Dead']);
...
for i = 1:numel(CHs)
    ...
    if(energy(head)>0)
        ...
        if dBS <= params.Tx_range
            EtxBS = params.Eelec*params.packetSize + params.Efs*dBS^params.alpha*params.packetSize;
        else
            EtxBS = params.Eelec*params.packetSize + params.Emp*dBS^params.alpha*params.packetSize;
        end
