    ...
    if operating_nodes <= n * 0.75 && temp_val == 1
        ...
        for i = 1:n
            if SN(i).condition == 1 % Alive node
                ...
                plot(SN(i).x, SN(i).y, 'bx', 'MarkerSize', 4, 'LineWidth', 2); % Black cross for dead nodes
            end
        end
        ...
        hDeadNode = plot(nan, nan, 'bx', 'MarkerSize', 4, 'LineWidth', 2); % Black cross for dead nodes
        hSink = plot(nan, nan, 'p', 'MarkerSize', 10, 'MarkerFaceColor', 'g', 'MarkerEdgeColor', 'g');
        hCLusterLink = plot(nan,nan, 'b-');
...
legend({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}, 'Location', 'BestOutside');
hold off;
...
legend(flip({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'}), 'Location', 'BestOutside');
hold off;
...
x = categorical({'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'});
x = reordercats(x, {'RCA Method', 'SRA Method', 'RRA Method', 'DBSCAN', 'HEED', 'LEACH-C', 'LEACH', 'LEACH-MCDM', 'DDMPEA-ANUM', 'DWEHC', 'FOHCA'});
...
for i = 1:n
    ...
    if (SN(i).Energy > 0)
        ...
        commRange = 50; % you can define it globally or pass if needed
        k_n = 0;
        for j = 1:n
            if i ~= j
...
while sum(energy > 0) > 0
    ...
    if rounds == 1
        ...
        legend({'Sensor Nodes','Sink','Cluster Heads','Cluster Links'}, 'Location', 'BestOutside');
        hold off;
    ...
    if quarterNodesDead_ddmpea == -1 && deadNodes >= 0.25 * (n - noise_points_count)
        ...
        plot(positions(deadNodesIdx,1), positions(deadNodesIdx,2), 'bx', 'MarkerSize', 4, 'LineWidth', 2);
...
while sum(energy > 0) > 0
    ...
    if rounds == 1
        firstCHs_dwehc = unique(result.head_id); % Save first CHs
        firstClusters_dwehc = result.neighbors;  % Save first Clusters
        figure(27);
        hold on;
        grid on;
        % axis([0 params.areaSize 0 params.areaSize]);
        ...
        % Plot Cluster Heads
        plot(positions(firstCHs_dwehc,1), positions(firstCHs_dwehc,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');

        % Draw lines from cluster members to their CH
        for i = 1:length(firstClusters_dwehc)
            members = firstClusters_dwehc{i};
            CH = firstCHs_dwehc(i);
            for j = 1:length(members)
                plot([positions(CH,1), positions(members(j),1)], [positions(CH,2), positions(members(j),2)], 'b-');
            end
    ...
    if quarterNodesDead_dwehc == -1 && deadNodes >= 0.25 * (n - noise_points_count)
        ...
        quarterCHs_dwehc = unique(result.head_id); % Save quarter-dead CHs
        quarterClusters_dwehc = result.neighbors;  % Save quarter-dead Clusters
        figure(28);
        hold on;
        grid on;
        % axis([0 params.areaSize 0 params.areaSize]);
        title('DWEHC Network Status When >= 1/4th Nodes Are Dead');
        ...
        % Plot Cluster Heads at 25% death
        plot(positions(quarterCHs_dwehc,1), positions(quarterCHs_dwehc,2), 'ro', 'MarkerSize', 7, 'MarkerFaceColor', 'r');
        ...
        % Draw lines from cluster members to their CH
        nClusters = min(length(quarterClusters_dwehc), length(quarterCHs_dwehc));
        for i = 1:nClusters
            members = quarterClusters_dwehc{i};
            CH = quarterCHs_dwehc(i);
            for j = 1:length(members)
                plot([positions(CH,1), positions(members(j),1)], [positions(CH,2), positions(members(j),2)], 'b-');
            end
        end
...
while sum(energy > 0) > 0
    ...
    if quarterNodesDead_fohca == -1 && deadNodes >= 0.25 * (n - noise_points_count)
        ...
        quarterCHs_fohca = CH;          % Save quarter-dead CHs
...
%% calculates energy for the DDMPEA-ANUM, DWEHC & FOHCA
